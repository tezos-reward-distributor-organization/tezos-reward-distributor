...
def getLiquidityProvidersList_tzstats(big_map_id):
    offset = 0
    listLPs = {}
    resp = ' '
    while resp != []:
        uri = 'http://api.tzstats.com/explorer/bigmap/{}/keys?limit=100&offset={}'.format(big_map_id, offset)
        offset += 100
        resp = requests.get(uri, timeout=5)
        if resp.status_code == 200:
            resp = resp.json()
            for item in resp:
                listLPs[item['key']] = item['key_hash']
    return listLPs
...
def get_dexter_balance_map(contract_id = 'KT1Puc9St8wdNoGtLiD2WXaHbWU7styaxYhD', snapshot_block = 'BMQn5rnV1U5snTAmocdqzBgtGWd9kpUYnGHTh9zBhVWm5Mh5e5v'):
    ...
    for LP in listLPs:
        balanceMap[LP] = {}
        balanceMap[LP]['liquidity_share'] = getBalanceFromBigMap_rpc(storage['big_map_id'], listLPs[LP], snapshot_block)
