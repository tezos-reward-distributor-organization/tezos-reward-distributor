...
class TelegramPlugin(plugins.Plugin):
    ...
    def postMessage(self, chatId, msg, msgType):
        ...
        verbose_logger.debug(
            "[TelegramPlugin] {:s} raw response: {:}".format(msgType, respJ)
        )
        if not respJ["ok"]:
            logger.error(
    ...
    def validateConfig(self):
        ...
        if "chat_ids" in cfg_keys:
            raise plugins.PluginConfigurationError(
                "[{:s}] 'chat_ids' no longer supported; Please see upgrading instructions.".format(
                    self.name
                )
            )
        ...
        if not isinstance(self.admin_chat_ids, list):
            raise plugins.PluginConfigurationError(
                "[{:s}] 'admin_chat_ids' must be in list format".format(self.name)
            )

        # Same for publicChatIds
        if not isinstance(self.payouts_chat_ids, list):
            raise plugins.PluginConfigurationError(
                "[{:s}] 'payouts_chat_ids' must be in list format".format(self.name)
            )
