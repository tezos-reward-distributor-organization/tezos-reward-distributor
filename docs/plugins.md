...
* send_admin: Whether or not to send administrative-style messages using the bot. These messages will appear alongside general user-style messages.
* telegram_text: This text will be sent to the channel configured for your bot. The text will be passed through a filter to replace the following placeholders: `%CYCLE%`, `%NDELEGATORS%`, `%TREWARDS%`. Markdown formatting is supported; Emojis and other unicode characters are supported.
