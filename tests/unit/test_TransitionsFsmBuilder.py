...
class TestFsmBuilder(TestCase):
    def test_add_initial_state(self):
        ...
        fsmBuilder.add_state(
            "middle", on_leave=lambda e: print("left middle state: message=", e.args)
        )
        ...
        fsmBuilder.add_final_state(
            ...
            on_enter=lambda e: print(
                "entered final state", e.kwargs.get("msg", "None")
            ),
        )
        fsmBuilder.add_transition(
            "move",
            "start",
            "middle",
        ...
        fsmBuilder.add_conditional_transition(
            "run", "middle", self.is_tired, "not_running", "running"
        )
        fsmBuilder.add_transition(
            "stop",
            ["not_running", "running"],
    ...
    def test_exceptions(self):
        ...
        fsmBuilder.add_final_state(
            "done",
            on_enter=lambda e: print(
                "entered final state", e.kwargs.get("msg", "None"), "error:", e.error
            ),
        )
